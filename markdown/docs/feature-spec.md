...
| **scrcpy起動成功** | 実行中 | `画面転送を開始しました。` | scrcpyウィンドウをアクティブ化 |
| **scrcpy起動失敗** | 起動エラー | `scrcpyの起動に失敗しました。設定を確認してください。` | 設定画面を開く（将来実装） |
| **複数デバイス検出** | デバイス選択が必要 | `複数のデバイスが検出されました。対象を1つ選択してください。` | デバイス選択UIを開く（将来実装） |
