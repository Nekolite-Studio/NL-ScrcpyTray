...
const useMockDeviceManager = () => {
    const [devices, setDevices] = useState<DeviceViewModel[]>([]); // Start with empty
    ...
    const updateDeviceOrder = (reorderedDevices: DeviceViewModel[]) => {
        const ids = reorderedDevices.map(d => d.id);
        (window as any).chrome.webview.hostObjects.bridge.updateDeviceOrder(ids);
        setDevices(reorderedDevices); // Optimistic update
    };
